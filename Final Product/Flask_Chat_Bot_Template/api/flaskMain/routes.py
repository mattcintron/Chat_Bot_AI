...
def uploader_file():
    if request.method == 'POST':
        ...
        #f.save(str(Path(__file__).absolute().parent /'/Scripts/json_files/')+secure_filename('/intents.json'))
        f.save(str(Path(__file__).absolute().parent /'Scripts'/'json_files'/'intents.json'))
        retrain_model()
...
def InteractiveFieldExample():
    ...
    if form.validate_on_submit() and mw != '':
        ...
        na = answers.copy()
        na.reverse()
        return render_template('interactiveFieldExample.html', title='Interactive Field Example', form=form, content = text,
                                q = nq, a = na, time =ts, score=scr, url =url)
    if respond(mw) == "Error":
